...
const browserUA = {
	...
	youkuLastVersion: userAgent.replace(/.*youku\/[^.]\.[^.]\.([0-9]+).*/i, '$1'),
	// 优酷pad版
	...
	// 判断是否为淘宝app
	isTaoBao: /aliapp\(tb/i.test(userAgent),
	// 判断是否为高德app
	inGaodeMap: /amap/i.test(userAgent);,
};

// (function(configs){
// 	configs.forEach(function( config ){
// 		browserUA["is" + config.name.charAt(0).toUpperCase() + config.name.replace(/^\w/,"")] = (function( kw ){
// 			if( typeof kw === "string" ){
// 				return new RegExp( kw , "ig").test( userAgent );
// 			}else if( kw instanceof Function ){
// 				return !!kw(userAgent);
// 			}else{
// 				//
// 			}
...
browserUA.version = parseInt(browserUA.youkuMainVersion)*1000 + parseInt(browserUA.youkuSubVersion)*100 +browserUA.youkuLastVersion*10;
...
//是否支持活动
browserUA.ifSupport = !((browserUA.version < 6620 && browserUA.iOS) || !browserUA.isWindvane);

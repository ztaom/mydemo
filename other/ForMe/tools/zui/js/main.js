jqapi = function() {
  ...
  function initialize() {
    ...
    elements.window.resize(function() {
      ...
      var listH = winH - values.searchHeight - 45;//var listH = winH - values.searchHeight
      
      elements.list.height(listH);
      elements.results.height(listH);
      //elements.content.height(winH);//这里暂时不能设定高度，在ie下面不能同步苏刷新
  ...
  function urlMethodName(link) {
    ...
    return href.substr(5, href.length - 16);//substr(5, href.length - 16)  目前可以取消这段代码
  } //-urlMethodName
  ...
  function startSearch() {
    elements.search.doTimeout('text-type', 300, function() {
      ...
      if(term.length) {
        ...
        elements.results.prepend(winner).highlight(term, true, 'red').children('li:first').addClass(values.selected);
